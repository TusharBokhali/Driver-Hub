...
export default function HomeScreens({ navigation }: any) {
  const [IsActive, setActive] = useState('Rent');
  ...
  const GetSellCar = async () => {
    try {
      ...
      if (IsActive === "Rent" || IsActive === "Sell") {
        params.vehicleType = IsActive.toLowerCase();
      }

      const res = await axios.get(Api.getCar, { params });
      ...
      console.error("GetSellCar API Error:", {
